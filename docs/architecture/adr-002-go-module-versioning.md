...
For example, changing the go package version from `v2` to `v3` bumps the import from `github.com/cosmos/ibc-go/v2` to `github.com/dojimanetwork/ibc-go/v3`.
...
Attempting to import two versions of ibc-go, such as `github.com/cosmos/ibc-go/v2` and `github.com/dojimanetwork/ibc-go/v3`, will result in multiple issues. 
...
`github.com/dojimanetwork/ibc-go/v3` cannot be imported with any previous ibc-go versions
