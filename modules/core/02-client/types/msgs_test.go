...
func (suite *TypesTestSuite) TestMarshalMsgUpdateClient() {
	...
	testCases := []struct {
		...
		{
			"dojimamint client", func() {
				msg, err = types.NewMsgUpdateClient("dojimamint", suite.chainA.CurrentTMClientHeader(), suite.chainA.SenderAccount.GetAddress().String())
				suite.Require().NoError(err)
...
func (suite *TypesTestSuite) TestMsgUpdateClient_ValidateBasic() {
	...
	cases := []struct {
		...
		{
			"invalid dojimamint header",
			func() {
				msg, err = types.NewMsgUpdateClient("dojimamint", &ibctmtypes.Header{}, suite.chainA.SenderAccount.GetAddress().String())
				suite.Require().NoError(err)
			},
			false,
		},
...
func (suite *TypesTestSuite) TestMarshalMsgUpgradeClient() {
	...
	testCases := []struct {
		...
		{
			"client upgrades to new dojimamint client",
			func() {
				tendermintClient := ibctmtypes.NewClientState(suite.chainA.ChainID, ibctesting.DefaultTrustLevel, ibctesting.TrustingPeriod, ibctesting.UnbondingPeriod, ibctesting.MaxClockDrift, clientHeight, commitmenttypes.GetSDKSpecs(), ibctesting.UpgradePath, false, false)
	...
	testCases := []struct {
		...
		},
		{
			"dojimamint client", func() {
				...
				misbehaviour := ibctmtypes.NewMisbehaviour("dojimamint", header1, header2)
				msg, err = types.NewMsgSubmitMisbehaviour("dojimamint", misbehaviour, suite.chainA.SenderAccount.GetAddress().String())
				suite.Require().NoError(err)
...
func (suite *TypesTestSuite) TestMsgSubmitMisbehaviour_ValidateBasic() {
	...
	cases := []struct {
		...
		{
			"invalid dojimamint misbehaviour",
			func() {
				msg, err = types.NewMsgSubmitMisbehaviour("dojimamint", &ibctmtypes.Misbehaviour{}, suite.chainA.SenderAccount.GetAddress().String())
				suite.Require().NoError(err)
			},
			false,
		},
