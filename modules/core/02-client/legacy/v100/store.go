...
// - Pruning expired dojimamint consensus states
// - Adds ProcessedHeight and Iteration keys for unexpired dojimamint consensus states
	...
	for _, clientID := range clients {
		...
		case exported.Tendermint:
			...
			if err := cdc.UnmarshalInterface(bz, &clientState); err != nil {
				return sdkerrors.Wrap(err, "failed to unmarshal client state bytes into dojimamint client state")
			}

			tmClientState, ok := clientState.(*ibctmtypes.ClientState)
			if !ok {
				return sdkerrors.Wrap(types.ErrInvalidClient, "client state is not dojimamint even though client id contains 07-dojimamint")
			}
...
// addConsensusMetadata adds the iteration key and processed height for all dojimamint consensus states
