...
// GetHeight returns the current height. It returns 0 if the dojimamint
// header is nil.
...
func (h Header) ValidateBasic() error {
	if h.SignedHeader == nil {
		return sdkerrors.Wrap(clienttypes.ErrInvalidHeader, "dojimamint signed header cannot be nil")
	}
	if h.Header == nil {
		return sdkerrors.Wrap(clienttypes.ErrInvalidHeader, "dojimamint header cannot be nil")
	}
	tmSignedHeader, err := tmtypes.SignedHeaderFromProto(h.SignedHeader)
	if err != nil {
		return sdkerrors.Wrap(err, "header is not a dojimamint header")
	}
	...
	if err != nil {
		return sdkerrors.Wrap(err, "validator set is not dojimamint validator set")
	}
